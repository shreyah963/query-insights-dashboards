...
import { ProfilerQueriesProps, QueryProfile, AggregationProfile } from '../types';
 ...
 */
export const QueryTree: React.FC<ProfilerQueriesProps> = ({
  queries,
  aggregations,
  selectedQuery,
  onQuerySelect,
  ...
  const renderTreeNode = (node: ProcessedQuery, depth = 0): React.ReactNode => {
    ...
    return (
      <div key={node.id} style={{ marginLeft: depth * 12 }}>
        <div
          onClick={() => onQuerySelect(node as any)}
          style={{
            display: 'flex',
            alignItems: 'center',
            padding: '4px 6px',
        ...
        {hasChildren && isExpanded && (
          <div>
            {node.children!.map((child) => renderTreeNode(child, depth + 1)) as any}
          </div>
        )}
      </div>
    );
    ...
    <div style={{ display: 'flex', height: '600px', border: '1px solid #D3DAE6', borderRadius: '6px' }}>
      ...
      >
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '4px' }}>
          {processedQueries.map((query) => renderTreeNode(query)) as any}
        </div>
      </div>
